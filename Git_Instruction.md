...
Для вывода на экран истории всех коммитов с их хеш-кодами выполняем команду `git log` (сокращенная версия истории `git log --oneline`). Для графического отображения веток используется команда `git log --graph` (сокращенная версия истории `git log --graph --oneline`).
...
## 11. Создание удаленного репозитория в GitHub.

Сначала необходимо создать аккаунт на сайте [GitHub.com](https://github.com/)

Для создания удаленного репозитория необходимо нажать "+" в правом верхнем углу окна и в всплывающем окне выбрать **New repository**, присвоить ему название, выбрать публичный или частный доступ к нему и при необходимости добавить файлы README.md и .getignore.

## 12. Работа с удаленным и локальным репозиториями.

Создаем локальный репозиторий. Для инициализации локального репозитория в терминале выполняем команду `git init`.

Создаем файл ***README.md*** и добавляем его к отслеживанию, выполняя команду `git add README.md`.

После добавления файла выполняем команду `git commit -m “initial commit”`.

Далее с помощью команды `git branch -M main` изменяем название основной ветки на *main* командой `branch -M main`. 
Это необходимо для того, чтобы основные ветки в локальном и удаленном резиториях соответствовали, т.к. основная ветка на GitHub по умолчанию *main*.

Привязать удаленный репозиторий к локальному можно командой `git remote add <название удалённого репозитория> <url-ссылка на удалённый репозиторий>`. Обычно в качестве названия удаленного репозитория используют `origin`, однако можно использовать любое другое название.

Для получения информации о привязке локального и удалённого репозиториев можно выполнить команду `git remote -v`.

Отправить локальный резозиторий в GitHub можно командой `git push -u origin main`.

Для получения изменений из удалённого репозитория используется команда `git pull`.

Для отправки изменений в удаленный репозиторий используется команда `git push`.

В случае, если необходимо отключить удаленный репозиторий от локального можно использовать команду `git remote remove origin`.

## 13. Работа с OpenSource проектами.

Для работы с OpenSource проектами необходимо сделать копию изначального репозитория, нажав на кнопку **Fork** в правом верхнем углу на странице данного репозитория в GitHub.

Для внесения своих изменений в проект необходимо его клонировать с помощью команды `git clone <url-ссылка на удалённый репозиторий>`.

В склонированном репозитории создаем новую ветку, в которой производим предлагаемые изменения.

С  помощью команды `git push` отправляем эти изменения в копию удаленного репозитория в GitHub, созданного через **Fork**, после чего можно отправить запрос (**Pull request**) автору изначального репозитория.

Для этого можно перейти на вкладку **Pull requests**, выбрать новую ветку и нажать кнопку **New pull request**. 

После создания **Pull request** автор получит изменения, которые может принять и влить в свою ветку или сообщить о необходимости доработки.

## 14. Удаление репозитория в GitHub.

В созданном аккаунте в GitHub открываем репозиторий, который мы хотим удалить.

Далее переходим на вкладку **Settings**.

В самом низу страницы в блоке *Danger Zone* нажимаем кнопку ***Delete this repository***.

Для подтверждения удаления репозитория необходимо вписать его название, нажать кнопку ***I understand the consequences, delete this repository*** и далее ввести пароль от аккаунта.
