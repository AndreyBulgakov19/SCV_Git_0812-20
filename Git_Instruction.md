...
* Переименовываем текущую ветку в ветку *main* при помощи команды `git branch -M main`
...
1. На GitHub нужно выбрать репозиторий, с который хотите поработать, и нажать на его страничке кнопку **Fork**. Репозиторий скопируется в ваш аккаунт.
2. На своем аккаунте в этом репозитории нажать кнопку **Code** и скопировать ссылку.
3. На своем компьютере в новой папке выполняем команду `git clone <url-адрес репозитория>`. После этого чужой репозиторий появится в вашей папке и с ним можно работать. Для этого нужно сначала перейти в папку репозитория при помощи команды `cd <название папки>` или нажав правой кнопкой мыши на папку и выбрать "Открыть во встроенном терминале".
4. Создаем новый файл, добавляем в него информацию и создаем новую ветку. **Важно**: Все измениния в чужом репозитории можно делать только на **новой ветке**! Переходим на эту ветку и уже на ней добавляем файл к отслеживанию и фиксируем изменения. 
5. Теперь необходимо отправить наши измениния на внешний репозиторий, но терминал не позволит выполнить сразу команду `git push`, так как на удаленном репозитории нашей новой ветки нет. Git подскажет команду, которую надо выполнить, это команда `git push --set-upstream origin <название ветки>`. Теперь новая ветка появилась на удаленном репозитории.
6. Отправим автору запрос на приняние наших изменений в его проект. Для этого заходим на GitHub в чужой репозиторий на нашем аккаунте и если появилась кнопка **Compare Pull Request**, используем ее. Если кнопка не появилась, выбираем **Pull requests**, далее **New Pull Request**, выбираем нашу новую ветку и нажимаем **Create Pull Request**. В открывшемся окне мы можем ввести сообщение для автора, после чего нажимаем **Create Pull Request**. 
