...
Если кнопка ```Create pull reqest``` не появилась, мы заходим во вкладку  ``` Pul reqest```, нажимаем ``` New pul reqest``` , во вкладке ``` compare``` выбираем новую ветку, и нажимаем ```Create pull reqest```, в новом окне мы так же можем написать сообщение автору, и нажимаем  ```Create pull reqest```. Нашь файл отправлен.
