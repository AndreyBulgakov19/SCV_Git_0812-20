...
1. Если локальный репозиторий ещё не создан, можно создать его из командной строки. для этого необходимо выполнить следующие команды:
```
...
``` 
2. Если репозиторий уже существует, необходимо просто связать его с удалённым с помощью следующих команд:
```
git remote add origin https://github.com/<Name_GitHub>/<Name_repository>
git branch -M main
git push -u origin main
...
Для внесения изменений создаётся новая ветка. после внесения необходимых изменений они фиксируюся и переносятся в репозиторий на Git Hub с помощью `git push`.

Далее нужно отправить внесённые изменения автору с помощью кнопки `pull request` . Изменения будут либо приняты автором, либо отклонены с соответствующими комментариями.
